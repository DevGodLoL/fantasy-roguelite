 ...
 * 
 * Run with: npx tsx scripts/e2e-season-test.ts
 */
...
import {
    ...
    awardPlayoffRewards
} from "../src/lib/game-data/playoffs";
...
async function simulateWeekSimple(leagueId: string, weekNumber: number) {
...
async function getSeasonStatus(leagueId: string) {
...
async function getPlayoffSeeds(leagueId: string) {
...
async function getMatchups(leagueId: string, weekNumber: number) {
    const week = await db.week.findUnique({
        where: { leagueId_number: { leagueId, number: weekNumber } },
        include: {
            matchups: {
...
async function main() {
    ...
    for (const team of seeds) {
        const bracket = (team.playoffSeed || 0) <= 6 ? '(Championship Bracket)' : '(Consolation Bracket)';
        log(`  #${team.playoffSeed} - ${team.name} ${bracket}`, (team.playoffSeed || 0) <= 2 ? 'green' : (team.playoffSeed || 0) <= 6 ? 'cyan' : 'magenta');
    }
    ...
    if (status?.championTeamId) {
        const champion = await db.team.findUnique({
    ...
    if (status?.consolationWinnerId) {
        const phoenix = await db.team.findUnique({
