...
import DraftCinematics from "./DraftCinematics";
...
export default async function DraftRoom({
    ...
    return (
        <div className="min-h-screen bg-[#030303] text-white font-sans overflow-hidden flex flex-col selection:bg-purple-500/30">
            {/* Cinematic Overlay - Only on initial load or empty draft logic potentially */}
            <DraftCinematics />
            {/* Animated Background */}
            ...
            {/* ═══════════════════════════════════════════════════════════════ */}
            <header className="relative z-30 border-b border-white/10 bg-black/60 backdrop-blur-xl shrink-0">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between px-4 lg:px-6 py-4 lg:py-0 lg:h-16 gap-4">
                    <div className="flex items-center justify-between lg:justify-start gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-xl bg-gradient-to-br from-purple-600 to-blue-600 flex items-center justify-center text-base lg:text-lg shadow-[0_0_20px_rgba(147,51,234,0.3)]">
                                    📜
                                </div>
                                <div>
                                    <h1 className="font-black uppercase tracking-tighter text-base lg:text-xl bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent">
                                        The Sacred Draft
                                    </h1>
                                    <div className="text-[9px] lg:text-[10px] text-zinc-500 uppercase tracking-widest leading-none mt-0.5">
                                        {draft.format} Format <span className="hidden sm:inline">• {numTeams} Commanders</span>
                                    </div>
                                </div>
                            </div>
                        </div>

                        {/* Status Badge - Mobile only in this slot */}
                        <div className={`
                            lg:hidden px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wide
                        ...
                        `}>
                            {draft.status === 'drafting' ? (isUserTurn ? 'YOUR TURN' : 'ACTIVE') : draft.status === 'completed' ? 'DONE' : 'WAIT'}
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between lg:justify-end gap-3 lg:gap-6">
                        ...
                        <div className="flex items-center gap-3">
                            <div className="text-right hidden sm:block">
                                ...
                                <div className="text-sm font-bold text-white leading-none mt-1">{picksMade} / {totalPicks} Chosen</div>
                            </div>
                            <div className="w-20 lg:w-24 h-1.5 lg:h-2 bg-zinc-800 rounded-full overflow-hidden">
                                <div
                        ...
                        <div className="px-3 py-1.5 lg:px-4 lg:py-2 bg-gradient-to-r from-purple-900/30 to-blue-900/30 border border-purple-500/30 rounded-xl">
                            <div className="text-[9px] lg:text-[10px] text-purple-400 uppercase tracking-widest font-bold">Round {Math.min(currentRound, TOTAL_ROUNDS)}</div>
                            <div className="text-sm lg:text-lg font-black text-white leading-none mt-0.5">Pick #{draft.currentPick}</div>
                        </div>
                        ...
                        <div className="flex gap-2">
                            {draft.status === 'drafting' && !isUserTurn && (
                                <form action={async () => {
                                    ...
                                    <button className="px-3 lg:px-5 py-2 lg:py-2.5 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white font-black text-[9px] lg:text-xs uppercase rounded-xl transition-all shadow-[0_0_20px_rgba(59,130,246,0.3)] animate-pulse">
                                        Summon <span className="hidden sm:inline">AI Picks</span>
                                    </button>
                                </form>
                            )}
                            {draft.status === 'pre_draft' && (
                                <form action={async () => {
                                    ...
                                    <button className="px-3 lg:px-5 py-2 lg:py-2.5 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white font-black text-[9px] lg:text-xs uppercase rounded-xl transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)]">
                                        ⚔️ Begin <span className="hidden sm:inline">the Ritual</span>
                                    </button>
                                </form>
                            )}
                            {draft.status === 'completed' && (
                                <Link
                                    ...
                                    className="px-3 lg:px-5 py-2 lg:py-2.5 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white font-black text-[9px] lg:text-xs uppercase rounded-xl transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)]"
                                >
                                    <span className="hidden sm:inline">View Campaign</span> →
                                </Link>
                            )}
                        </div>
                        ...
                        <div className={`
                            hidden lg:block px-4 py-2 rounded-xl text-sm font-black uppercase tracking-wide
                            ${draft.status === 'drafting'
                                ? isUserTurn
            ...
            {/* MAIN CONTENT */}
            {/* ═══════════════════════════════════════════════════════════════ */}
            <div className="flex-1 flex flex-col lg:flex-row lg:overflow-hidden relative z-10 overflow-y-auto lg:overflow-y-hidden">
                {/* Left: Available Souls (Players) */}
                <div className="h-[350px] lg:h-full lg:w-[300px] shrink-0 border-b lg:border-b-0 lg:border-r border-white/10">
                    <DraftPlayerList
                ...
                <main className="flex-1 overflow-x-auto lg:overflow-auto p-4 border-b lg:border-b-0 border-white/10 custom-scrollbar">
                ...
                <aside className="h-[40vh] lg:h-full lg:w-[240px] border-t lg:border-t-0 lg:border-l border-white/10 bg-black/40 backdrop-blur-sm shrink-0 overflow-y-auto">
                    <div className="sticky top-0 z-20 p-4 border-b border-white/10 bg-purple-500/5 backdrop-blur-md">
                        <div className="flex items-center gap-3">
