...
export async function checkWeekReadiness(leagueId: string, weekNumber: number) {
    const week = await db.week.findUnique({
        where: { leagueId_number: { leagueId, number: weekNumber } },
        include: {
            matchups: {
                include: {
                    homeTeam: { include: { rosterSlots: true, powerups: { where: { weekId: null, isConsumed: false } } } },
                    awayTeam: { include: { rosterSlots: true, powerups: { where: { weekId: null, isConsumed: false } } } },
                },
            },
        },
    });
    ...
    const warnings: { teamName: string; issue: string }[] = [];

    for (const matchup of week.matchups) {
        const checkTeam = (team: any) => {
            // Check for empty starter slots
            const emptyStarters = team.rosterSlots.filter((s: any) => s.slotType !== 'BENCH' && !s.playerId);
            if (emptyStarters.length > 0) {
                warnings.push({
                    teamName: team.name,
                    issue: `Has ${emptyStarters.length} empty starter slots (${emptyStarters.map((s: any) => s.slotType).join(', ')}).`
                });
            }

            // Check for unassigned artifacts (if they have artifacts but none assigned to this week)
            // Note: In this game, artifacts are auto-assigned to the week they are picked, 
            // but some might be "permanent" or "floating". Let's check for unconsumed cards.
            const unconsumedArtifacts = team.powerups.filter((p: any) => !p.isConsumed);
            if (unconsumedArtifacts.length > 0) {
                // Only warn if they haven't picked a card FOR THIS WEEK specifically yet?
                // Actually, let's keep it simple: if you have unconsumed artifacts, we warn.
            }
        };

        checkTeam(matchup.homeTeam);
        checkTeam(matchup.awayTeam);
    }
...
export async function simulateWeek(leagueId: string, weekNumber: number, bypassReadiness = false) {
    try {
        if (!bypassReadiness) {
            const readiness = await checkWeekReadiness(leagueId, weekNumber);
            if (!readiness.isReady) {
                return { success: false, error: "Lineup anomalies detected", needsConfirmation: true, warnings: readiness.warnings };
            }
        }
