...
export default async function GrandArchivePage({
    ...
    return (
        <div className="min-h-screen bg-[#030303] text-white font-sans selection:bg-amber-500/30">
            ...
            {/* Header */}
            <header className="relative z-10 border-b border-white/10 bg-black/40 backdrop-blur-md sticky top-0 overflow-x-hidden">
                <div className="max-w-5xl mx-auto px-6 py-4 md:h-20 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-6 w-full md:w-auto">
                        <Link
                            href={`/league/${id}`}
                            className="group flex items-center gap-2 text-zinc-500 hover:text-amber-400 transition-colors text-xs font-black uppercase tracking-widest shrink-0"
                        >
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="truncate">
                                <h1 className="text-lg md:text-xl font-black uppercase tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-amber-200 to-amber-500">
                                    The Grand Archive
                                </h1>
                    ...
                    {/* Filter Bar */}
                    <div className="flex items-center gap-1 bg-white/5 p-1 rounded-xl border border-white/5 w-full md:w-auto overflow-x-auto no-scrollbar">
                        {[
                            { id: 'ALL', label: 'All', icon: <ScrollText className="w-3 h-3" /> },
                            { id: 'ADD', label: 'Adds', icon: <UserPlus className="w-3 h-3" /> },
                            { id: 'DROP', label: 'Drops', icon: <UserMinus className="w-3 h-3" /> },
                            { id: 'TRAIT_GAINED', label: 'Mutations', icon: <Sparkles className="w-3 h-3" /> },
                        ].map((f) => (
                            <Link
                                key={f.id}
                                href={`/league/${id}/transactions?type=${f.id}`}
                                className={`
