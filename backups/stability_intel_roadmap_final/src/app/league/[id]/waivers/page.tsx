...
export default async function WaiversPage({
    ...
    return (
        <div className="min-h-screen bg-[#030303] text-white font-sans flex flex-col selection:bg-purple-500/30">
            ...
            {/* ═══════════════════════════════════════════════════════════════ */}
            {/* HEADER - MARKET COMMAND */}
            <header className="relative z-30 min-h-[5rem] lg:h-20 border-b border-white/10 bg-black/60 backdrop-blur-xl sticky top-0 shrink-0">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between px-4 lg:px-8 py-4 lg:py-0 h-full gap-4">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 lg:gap-6">
                        <Link href={`/league/${leagueId}`} className="text-zinc-500 hover:text-purple-400 transition-all flex items-center gap-2 group text-sm">
                            <span className="group-hover:-translate-x-1 transition-transform">←</span>
                            Back <span className="hidden sm:inline">to Command</span>
                        </Link>
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-base lg:text-xl font-black uppercase tracking-tighter bg-gradient-to-r from-white via-white to-zinc-400 bg-clip-text text-transparent">Mercenary Marketplace</h1>
                                <div className="text-[9px] lg:text-[10px] text-zinc-500 uppercase tracking-widest font-bold leading-none mt-1">Recruit new souls for your legion</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between lg:justify-end gap-6 border-t lg:border-t-0 border-white/5 pt-4 lg:pt-0">
                        ...
                        <div className="flex flex-col items-start lg:items-end">
                            ...
                            <div className="text-lg lg:text-xl font-black text-blue-400 font-mono italic leading-none mt-1">${userTeam.faabBalance}</div>
                        </div>
                        ...
                        <form action={async () => {
                            ...
                            <button className="whitespace-nowrap px-4 py-2 lg:px-6 lg:py-2.5 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white font-black uppercase text-[10px] lg:text-xs rounded-xl transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)]">
                                Execute <span className="hidden sm:inline">Contracts Now</span>
                            </button>
                        </form>
                    </div>
                </div>
