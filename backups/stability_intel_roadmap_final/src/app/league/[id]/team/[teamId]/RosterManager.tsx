                        ...
                        {isSelected ? (
                            ...
                            slot.player && !readOnly && !selectedSlotId && (
                                <button
                                    onClick={async (e) => {
                                        e.stopPropagation();
                                        if (dropConfirmId !== slot.id) {
                                            setDropConfirmId(slot.id);
                                            setTimeout(() => setDropConfirmId(null), 3000); // Reset after 3 seconds
                                            return;
                                        }
                                    ...
                                    className={`
                                        px-3 py-1.5 text-[10px] font-bold rounded-lg transition-all uppercase tracking-tighter border
                                        ${dropConfirmId === slot.id
                                            ? 'bg-red-600 text-white border-red-400 animate-pulse shadow-[0_0_15px_rgba(220,38,38,0.4)]'
                                            : 'text-zinc-600 hover:text-red-400 hover:bg-red-500/10 border-transparent hover:border-red-500/30'
                                        }
                                    `}
                                >
                                    {dropConfirmId === slot.id ? 'Breaking Seal (Click Again)' : 'Release'}
                                </button>
                            )
                        )}
                    </div>
