...
import { Lock, Swords, CheckCircle2, Trophy, Sparkles, Crown, Flame, Shield, Skull, Info, User, Brain } from "lucide-react";
import { PERSONALITIES, type Archetype } from "@/lib/game-data/personalities";
...
export default function CampaignMapView({ leagueId, weeks, stats, teamName }: CampaignMapViewProps) {
    ...
    const FloorCard = ({ week, isPlayoff = false }: { week: WeekNode; isPlayoff?: boolean }) => {
        ...
        const opponent = week.matchup?.opponent;
        const personality = opponent ? PERSONALITIES[opponent.archetype as Archetype] : null;
            ...
            <div className="group relative">
                <Link
                    ...
                    className={`
                        ...
                        ${week.isLocked ? 'cursor-not-allowed opacity-60' : 'hover:scale-[1.01] hover:z-10'}
                    `}
                >
                    {/* Floor Number Badge */}
                    <div className={`
                    ...
                    <div className="text-right shrink-0">
                        {week.isCompleted && (
                            <div className="flex flex-col items-end">
                                ...
                                <span className="text-[10px] font-mono text-zinc-500">{week.matchup?.homeScore.toFixed(0)} - {week.matchup?.awayScore.toFixed(0)}</span>
                            </div>
                        )}
                        {week.isCurrent && (
                ...
                {!week.isLocked && (
                    <div className={`
                        absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-60 p-4 
                        bg-[#0a0a0a] border border-white/10 rounded-2xl shadow-2xl 
                        opacity-0 translate-y-2 pointer-events-none z-50
                    ...
                    `}>
                        ...
                        <div className="relative z-10">
                            <div className="flex items-center justify-between mb-3 border-b border-white/5 pb-2">
                                <div className="text-[10px] font-black uppercase text-zinc-500 tracking-widest flex items-center gap-1.5">
                                    <Brain size={12} className="text-purple-400" />
                                    Intel Report
                                </div>
                                {week.isCompleted && (
                                    <div className="text-[10px] font-black text-emerald-400">SUCCESS</div>
                                )}
                            </div>
                            ...
                            {opponent ? (
                                <div className="space-y-3">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-lg bg-zinc-900 border border-white/5 flex items-center justify-center text-xl`}>
                                            {personality?.icon}
                                        </div>
                                        <div>
                                            <div className="text-xs font-black text-white">{opponent.name}</div>
                                            <div className={`text-[9px] font-bold uppercase tracking-wider ${personality?.color}`}>
                                                {personality?.title}
                                            </div>
                                        </div>
                                    </div>
                                    <p className="text-[10px] text-zinc-400 leading-relaxed italic">
                                        "{personality?.quotes[0]}"
                                    </p>
                                    <div className="grid grid-cols-2 gap-2 pt-2">
                                        <div className="bg-white/5 rounded-lg p-2 border border-white/5">
                                            <div className="text-[8px] text-zinc-500 font-bold uppercase">Strategy</div>
                                            <div className="text-[9px] text-zinc-300 font-medium">Aggressive</div>
                                        </div>
                                        <div className="bg-white/5 rounded-lg p-2 border border-white/5">
                                            <div className="text-[8px] text-zinc-500 font-bold uppercase">Risk Level</div>
                                            <div className="text-[9px] text-red-400 font-black">HIGH</div>
                                        </div>
                                    </div>
                                </div>
                            ) : (
                        ...
                        {/* Tooltip Arrow */}
                        <div className="absolute top-full left-1/2 -translate-x-1/2 -translate-y-[2px] w-3 h-3 bg-[#0a0a0a] border-r border-b border-white/10 rotate-45" />
                    </div>
                )}
            </div>
        );
